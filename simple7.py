...
prompt_template_resto = PromptTemplate(
    ...
    template="think you are a data scientist at an f1 team, given the season: {season}, round: {round}, total number of laps: {total_no_of_laps}, randomness factor: {randomness_factor} and model selection: {model_selection}, give me the details(circuit name, location, number of laps, circuit length, race distance, lap record, nature of the track and optimal theoretical tyre strategy using soft, medium and hard tyres) of the circuit corresponding to one of the rounds(eg. in 2024, 1 is bahrain, 24 is abu dhabi) and then an example lap-by-lap prediction containing driver code, driver name, position, predicted lap to pit, laps till pitting, tyre degradatiion, fuel consumption rates, mgu-h and mgu-k usage, status and laptime of ALL the 20 drivers at a given race (give me a a direct table prediction without any code or theory)"
)
