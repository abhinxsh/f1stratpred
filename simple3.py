...
prompt_template_resto = PromptTemplate(
    input_variables=['season', 'round', 'total_no_of_laps', 'model_selection'],
    template="think you are a data scientist at mercedes f1 team, given the season: {season}, round: {round}, total number of laps: {total_no_of_laps}, and model selection: {model_selection}, give me an example lap-by-lap prediction containing driver name, position, laps till pitting, status and laptime at bahrain gp in 2024 (give me a direct table prediction without any code or theory)"
)
...
with st.form(key='prediction_form'):
    season = st.selectbox("Season", [1960, 1961, 2019, 2020, 2021, 2022, 2023, 2024], key='season')
...
if submit_button:
    if all([season, round_number, total_laps, model_selection]):
        input_data = {
            'season': season,
            'round': round_number,
